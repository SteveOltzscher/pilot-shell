...
`)[2]||"").match(/at\s+(?:.*\s+)?\(?([^:]+):(\d+):(\d+)\)?/),p=l?`${l[1].split("/").pop()}:${l[2]}`:"unknown",m={...s,location:p};return this.warn(e,`[HAPPY-PATH] ${t}`,m,n),o}},u=new Y;var $=class{db;constructor(e=ce){e!==":memory:"&&le(y),this.db=new me.Database(e),this.db.run("PRAGMA journal_mode = WAL"),this.db.run("PRAGMA synchronous = NORMAL"),this.db.run("PRAGMA foreign_keys = ON"),this.initializeSchema(),this.ensureWorkerPortColumn(),this.ensurePromptTrackingColumns(),this.removeSessionSummariesUniqueConstraint(),this.addObservationHierarchicalFields(),this.makeObservationsTextNullable(),this.createUserPromptsTable(),this.ensureDiscoveryTokensColumn(),this.createPendingMessagesTable(),this.renameSessionIdColumns(),this.repairSessionIdColumnRename(),this.addFailedAtEpochColumn(),this.ensureSessionPlansTable(),this.createProjectRootsTable(),this.ensureNotificationsTable(),this.addSessionAgentColumn()}initializeSchema(){this.db.run(`
    ...
    `).all(e)}getObservationById(e){return this.db.prepare(`
      ...
      FROM observations
      WHERE id = ?
    `).get(e)||null}getObservationsByIds(e,t={}){if(e.length===0)return[];let{orderBy:s="date_desc",limit:n,project:o,type:i,concepts:a,files:d}=t,l=s==="date_asc"?"ASC":"DESC",p=n!=null?Math.trunc(Number(n)):void 0,m=p&&p>0?"LIMIT ?":"",T=e.map(()=>"?").join(","),_=[...e],E=[];if(o&&(E.push("project = ?"),_.push(o)),i)if(Array.isArray(i)){let h=i.map(()=>"?").join(",");E.push(`type IN (${h})`),_.push(...i)}else E.push("type = ?"),_.push(i);if(a){let h=Array.isArray(a)?a:[a],f=h.map(()=>"EXISTS (SELECT 1 FROM json_each(concepts) WHERE value = ?)");_.push(...h),E.push(`(${f.join(" OR ")})`)}if(d){let h=Array.isArray(d)?d:[d],f=h.map(()=>"(EXISTS (SELECT 1 FROM json_each(files_read) WHERE value LIKE ?) OR EXISTS (SELECT 1 FROM json_each(files_modified) WHERE value LIKE ?))");h.forEach(S=>{_.push(`%${S}%`,`%${S}%`)}),E.push(`(${f.join(" OR ")})`)}let O=E.length>0?`WHERE id IN (${T}) AND ${E.join(" AND ")}`:`WHERE id IN (${T})`;return p&&p>0&&_.push(p),this.db.prepare(`
      SELECT *
      FROM observations
      ${O}
    ...
    `).run(e,t,s.type,s.title,s.subtitle,JSON.stringify(s.facts),s.narrative,JSON.stringify(s.concepts),JSON.stringify(s.files_read),JSON.stringify(s.files_modified),n||null,o,d,a);return{id:Number(p.lastInsertRowid),createdAtEpoch:a}}storeSummary(e,t,s,n,o=0,i){let a=i??Date.now(),d=new Date(a).toISOString(),p=this.db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(e,t,s.request,s.investigated,s.learned,s.completed,s.next_steps,s.notes,n||null,o,d,a);return{id:Number(p.lastInsertRowid),createdAtEpoch:a}}storeObservations(e,t,s,n,o,i=0,a){let d=a??Date.now(),l=new Date(d).toISOString();return this.db.transaction(()=>{let m=[],T=this.db.prepare(`
        INSERT INTO observations
        (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
         files_read, files_modified, prompt_number, discovery_tokens, created_at, created_at_epoch)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);for(let E of s){let O=T.run(e,t,E.type,E.title,E.subtitle,JSON.stringify(E.facts),E.narrative,JSON.stringify(E.concepts),JSON.stringify(E.files_read),JSON.stringify(E.files_modified),o||null,i,l,d);m.push(Number(O.lastInsertRowid))}let _=null;if(n){let O=this.db.prepare(`
          INSERT INTO session_summaries
          (memory_session_id, project, request, investigated, learned, completed,
           next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(e,t,n.request,n.investigated,n.learned,n.completed,n.next_steps,n.notes,o||null,i,l,d);_=Number(O.lastInsertRowid)}return{observationIds:m,summaryId:_,createdAtEpoch:d}})()}storeObservationsAndMarkComplete(e,t,s,n,o,i,a,d=0,l){let p=l??Date.now(),m=new Date(p).toISOString();return this.db.transaction(()=>{let _=[],E=this.db.prepare(`
        INSERT INTO observations
        (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
         files_read, files_modified, prompt_number, discovery_tokens, created_at, created_at_epoch)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);for(let h of s){let f=E.run(e,t,h.type,h.title,h.subtitle,JSON.stringify(h.facts),h.narrative,JSON.stringify(h.concepts),JSON.stringify(h.files_read),JSON.stringify(h.files_modified),a||null,d,m,p);_.push(Number(f.lastInsertRowid))}let O;if(n){let f=this.db.prepare(`
          INSERT INTO session_summaries
          (memory_session_id, project, request, investigated, learned, completed,
           next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(e,t,n.request,n.investigated,n.learned,n.completed,n.next_steps,n.notes,a||null,d,m,p);O=Number(f.lastInsertRowid)}return this.db.prepare(`
      ...
      `).run(p,o),{observationIds:_,summaryId:O,createdAtEpoch:p}})()}getSessionSummariesByIds(e,t={}){if(e.length===0)return[];let{orderBy:s="date_desc",limit:n,project:o}=t,i=s==="date_asc"?"ASC":"DESC",a=n!=null?Math.trunc(Number(n)):void 0,d=a&&a>0?"LIMIT ?":"",l=e.map(()=>"?").join(","),p=[...e],m=o?`WHERE id IN (${l}) AND project = ?`:`WHERE id IN (${l})`;return o&&p.push(o),a&&a>0&&p.push(a),this.db.prepare(`
      SELECT * FROM session_summaries
      ${m}
      ORDER BY created_at_epoch ${i}
      ${d}
    `).all(...p)}getUserPromptsByIds(e,t={}){if(e.length===0)return[];let{orderBy:s="date_desc",limit:n,project:o}=t,i=s==="date_asc"?"ASC":"DESC",a=n!=null?Math.trunc(Number(n)):void 0,d=a&&a>0?"LIMIT ?":"",l=e.map(()=>"?").join(","),p=[...e],m=o?"AND s.project = ?":"";return o&&p.push(o),a&&a>0&&p.push(a),this.db.prepare(`
      SELECT
        up.*,
        s.project,
        s.memory_session_id
      ...
      ${d}
    `).all(...p)}getTimelineAroundTimestamp(e,t=10,s=10,n){return this.getTimelineAroundObservation(null,e,t,s,n)}getTimelineAroundObservation(e,t,s=10,n=10,o){let i=o?"AND project = ?":"",a=o?[o]:[],d,l;if(e!==null){let g=`
      ...
      `,h=`
        ...
        ORDER BY id ASC
        LIMIT ?
      `;try{let f=this.db.prepare(g).all(e,...a,s+1),S=this.db.prepare(h).all(e,...a,n+1);if(f.length===0&&S.length===0)return{observations:[],sessions:[],prompts:[]};d=f.length>0?f[f.length-1].created_at_epoch:t,l=S.length>0?S[S.length-1].created_at_epoch:t}catch(f){return u.error("DB","Error getting boundary observations",void 0,{error:f,project:o}),{observations:[],sessions:[],prompts:[]}}}else{let g=`
      ...
      `,h=`
        ...
        ORDER BY created_at_epoch ASC
        LIMIT ?
      `;try{let f=this.db.prepare(g).all(t,...a,s),S=this.db.prepare(h).all(t,...a,n+1);if(f.length===0&&S.length===0)return{observations:[],sessions:[],prompts:[]};d=f.length>0?f[f.length-1].created_at_epoch:t,l=S.length>0?S[S.length-1].created_at_epoch:t}catch(f){return u.error("DB","Error getting boundary timestamps",void 0,{error:f,project:o}),{observations:[],sessions:[],prompts:[]}}}let p=`
      SELECT *
      FROM observations
    ...
    `,_=this.db.prepare(p).all(d,l,...a),E=this.db.prepare(m).all(d,l,...a),O=this.db.prepare(T).all(d,l,...a);return{observations:_,sessions:E.map(g=>({id:g.id,memory_session_id:g.memory_session_id,project:g.project,request:g.request,completed:g.completed,next_steps:g.next_steps,created_at:g.created_at,created_at_epoch:g.created_at_epoch})),prompts:O.map(g=>({id:g.id,content_session_id:g.content_session_id,prompt_number:g.prompt_number,prompt_text:g.prompt_text,project:g.project,created_at:g.created_at,created_at_epoch:g.created_at_epoch}))}}getPromptById(e){return this.db.prepare(`
      SELECT
        p.id,
...
Do not output anything other than the summary content formatted in the XML structure above.`}},R=class r{static instance=null;activeMode=null;constructor(){}static getInstance(){return r.instance||(r.instance=new r),r.instance}loadMode(){return this.activeMode=he,he}getActiveMode(){if(!this.activeMode)throw new Error("No mode loaded. Call loadMode() first.");return this.activeMode}getObservationTypes(){return this.getActiveMode().observation_types}getObservationConcepts(){return this.getActiveMode().observation_concepts}getTypeIcon(e){return this.getObservationTypes().find(s=>s.id===e)?.emoji||"\u{1F4DD}"}getWorkEmoji(e){return this.getObservationTypes().find(s=>s.id===e)?.work_emoji||"\u{1F4DD}"}validateType(e){return this.getObservationTypes().some(t=>t.id===e)}getTypeLabel(e){return this.getObservationTypes().find(s=>s.id===e)?.label||e}};function K(r){let e=(r.title?.length||0)+(r.subtitle?.length||0)+(r.narrative?.length||0)+JSON.stringify(r.facts||[]).length;return Math.ceil(e/Te)}function J(r){let e=r.length,t=r.reduce((i,a)=>i+K(a),0),s=r.reduce((i,a)=>i+(a.discovery_tokens||0),0),n=s-t,o=s>0?Math.round(n/s*100):0;return{totalObservations:e,totalReadTokens:t,totalDiscoveryTokens:s,savings:n,savingsPercent:o}}function Ct(r){return R.getInstance().getWorkEmoji(r)}function v(r,e){let t=K(r),s=r.discovery_tokens||0,n=Ct(r.type),o=s>0?`${n} ${s.toLocaleString()}`:"-";return{readTokens:t,discoveryTokens:s,discoveryDisplay:o,workEmoji:n}}function j(r){return r.showReadTokens||r.showWorkTokens||r.showSavingsAmount||r.showSavingsPercent}var fe=C(require("path"),1),be=require("os"),X=require("fs");function z(r,e,t){let s=Array.from(t.observationTypes),n=s.map(()=>"?").join(","),o=Array.from(t.observationConcepts),i=o.map(()=>"?").join(",");return r.db.prepare(`
  ...
  `).all(e,...s,...o,t.totalObservationCount)}function Q(r,e,t){return r.db.prepare(`
    ...
    ORDER BY created_at_epoch DESC
    LIMIT ?
  `).all(e,t.sessionCount+x)}function Oe(r,e,t){let s=Array.from(t.observationTypes),n=s.map(()=>"?").join(","),o=Array.from(t.observationConcepts),i=o.map(()=>"?").join(","),a=e.map(()=>"?").join(",");return r.db.prepare(`
    SELECT
      id, memory_session_id, type, title, subtitle, narrative,
      facts, concepts, files_read, files_modified, discovery_tokens,
  ...
  `).all(...e,...s,...o,t.totalObservationCount)}function Se(r,e,t){let s=e.map(()=>"?").join(",");return r.db.prepare(`
    ...
    ORDER BY created_at_epoch DESC
    LIMIT ?
  `).all(...e,t.sessionCount+x)}function Ie(r,e,t,s){let n=Array.from(t.observationTypes),o=n.map(()=>"?").join(","),i=Array.from(t.observationConcepts),a=i.map(()=>"?").join(",");return r.db.prepare(`
    SELECT
  ...
  `).all(e,...n,...i,s,t.totalObservationCount)}function Re(r,e,t,s){return r.db.prepare(`
    ...
    LIMIT ?
  `).all(e,s,t.sessionCount+x)}function ye(r,e,t,s){let n=Array.from(t.observationTypes),o=n.map(()=>"?").join(","),i=Array.from(t.observationConcepts),a=i.map(()=>"?").join(","),d=e.map(()=>"?").join(",");return r.db.prepare(`
    SELECT
      o.id, o.memory_session_id, o.type, o.title, o.subtitle, o.narrative,
      o.facts, o.concepts, o.files_read, o.files_modified, o.discovery_tokens,
...
`):null}function Mt(r,e,t,s,n,o){let i=[];o?i.push(...Ke(r)):i.push(...we(r));let a=null,d="",l=!1;for(let p of e)if(p.type==="summary"){l&&(i.push(""),l=!1,a=null,d="");let m=p.data,T=ot(m.displayTime);o?i.push(...Ze(m,T)):i.push(...$e(m,T))}else{let m=p.data,T=dt(m.files_modified,n,m.files_read),_=it(m.created_at),E=_!==d,O=E?_:"";d=_;let g=t.has(m.id);if(T!==a&&(l&&i.push(""),o?i.push(...Je(T)):i.push(...Ue(T)),a=T,l=!0),g){let h=xt(m,s);o?i.push(...Qe(m,_,E,h,s)):(l&&!o&&(i.push(""),l=!1),i.push(...Fe(m,O,h,s)),a=null)}else o?i.push(ze(m,_,E,s)):i.push(Pe(m,O,s))}return l&&i.push(""),i}function ct(r,e,t,s,n){let o=[],i=Dt(r);for(let[a,d]of i)o.push(...Mt(a,d,e,t,s,n));return o}function lt(r,e,t){return!(!r.showLastSummary||!e||!!!(e.investigated||e.learned||e.completed||e.next_steps)||t&&e.created_at_epoch<=t.created_at_epoch)}function pt(r,e){let t=[];return e?(t.push(...k("Investigated",r.investigated,c.blue)),t.push(...k("Learned",r.learned,c.yellow)),t.push(...k("Completed",r.completed,c.green)),t.push(...k("Next Steps",r.next_steps,c.magenta))):(t.push(...M("Investigated",r.investigated)),t.push(...M("Learned",r.learned)),t.push(...M("Completed",r.completed)),t.push(...M("Next Steps",r.next_steps))),t}function ut(r,e){return e?et(r):je(r)}function mt(r,e,t){return!j(e)||r.totalDiscoveryTokens<=0||r.savings<=0?[]:t?tt(r.totalDiscoveryTokens,r.totalReadTokens):Xe(r.totalDiscoveryTokens,r.totalReadTokens)}var kt=_t.default.join((0,Et.homedir)(),".claude","plugins","marketplaces","pilot","plugin",".install-version");function wt(){try{return new $}catch(r){if(r.code==="ERR_DLOPEN_FAILED"){try{(0,gt.unlinkSync)(kt)}catch(e){u.debug("SYSTEM","Marker file cleanup failed (may not exist)",{},e)}return u.error("SYSTEM","Native module rebuild needed - restart Claude Code to auto-fix"),null}throw r}}function Ut(r,e){return e?st(r):We(r)}function Pt(r,e,t,s,n,o,i){let a=[],d=J(e);a.push(...rt(r,d,s,i));let l=t.slice(0,s.sessionCount),p=Ne(l,t),m=ee(e,p),T=Ce(e,s.fullObservationCount);a.push(...ct(m,T,s,n,i));let _=t[0],E=e[0];lt(s,_,E)&&a.push(...pt(_,i));let O=Z(e,s,o,n);return a.push(...ut(O,i)),a.push(...mt(d,s,i)),a.join(`
`).trimEnd()}async function se(r,e=!1){let t=q(),s=r?.cwd??process.cwd(),n=_e(s),o=r?.projects||[n],i=wt();if(!i)return"";try{let a=r?.planPath,d,l;return a?(d=o.length>1?ye(i,o,t,a):Ie(i,n,t,a),l=o.length>1?Le(i,o,t,a):Re(i,n,t,a)):(d=o.length>1?Oe(i,o,t):z(i,n,t),l=o.length>1?Se(i,o,t):Q(i,n,t)),d.length===0&&l.length===0?Ut(n,e):Pt(n,d,l,t,s,r?.session_id,e)}finally{i.close()}}0&&(module.exports={generateContext});
